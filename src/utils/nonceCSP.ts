...
export function nonceCSP(Astro: AstroGlobal) {
	const nonce = crypto.randomUUID().replace(/-/g, ""); // Generamos el nonce

	// Configuramos Content Security Policy para la respuesta
	...
	// Retornamos el nonce para que pueda ser usado en el layout y otros componentes
	return { nonce };
}
